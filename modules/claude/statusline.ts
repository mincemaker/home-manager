...
const rawText = new TextDecoder().decode(buf.subarray(0, n ?? 0));
if (!rawText.trim()) Deno.exit(0);
...
let cache: any = {};
try { cache = JSON.parse(await Deno.readTextFile(CACHE_FILE)); } catch { /* ignore */ }
...
if (hasRateData || hasContextData) {
  ...
  try { await Deno.writeTextFile(CACHE_FILE, JSON.stringify(newCache)); } catch { /* ignore */ }
